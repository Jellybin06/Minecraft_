...
public class Aokizi extends AbilityBase
{
    public Aokizi()
    {
        InitAbility("아오키지", AbilityBase.Type.Active_Immediately,
                AbilityBase.Rank.S, new String[] {
                        "철괴로 왼쪽클릭시 자신이 보고있는 방향으로 얼음을 날립니다.",
                        "철괴로 오른쪽클릭시 자신이 바라보고 있는 5칸 이내의 물을 얼려버립니다." });
        InitAbility(1, 0, true, AbilityBase.ShowText.Custom_Text);
        ...
        for (int i = 1; i < 10; i++) {
            ...
            if (l2.getWorld().getBlockAt(l2).getType() != Material.ICE)
                timer.schedule(new ExplosionTimer2(l2.getWorld().getBlockAt(l2)
                        .getTypeId(), l2.getWorld().getBlockAt(l2)), 988L);
            l2.getWorld().getBlockAt(l2).setType(Material.ICE);
            Player[] List = Bukkit.getOnlinePlayers();
            Player[] arrayOfPlayer1; int j = (arrayOfPlayer1 = List).length; for (int k = 0; k < j; k++) { Player p = arrayOfPlayer1[k];
                if (p != GetPlayer()) {
    ...
    class ExplosionTimer extends TimerTask {
        ...
        ExplosionTimer(int blockid, Block block) { this.world = block.getWorld();
    ...
    class ExplosionTimer2 extends TimerTask {
        ...
        public void run() {
            this.world.getBlockAt(this.location).setTypeId(this.blockd);
        }
    }
}
